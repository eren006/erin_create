...
cmd_view_schedule.help = "。时间线 [天]  \n无参数：查看日程概览；指定天（如 D3）：以合并转发查看当天详细日程。";
...
cmd_view_schedule.solve = (ctx, msg) => {
  ...
  if (!myRoleName) {
    seal.replyToSender(ctx, msg, "请先绑定角色");
    return seal.ext.newCmdExecuteResult(true);
  }
    ...
    if (isRecipient || isSender) {
      ...
      if (!alreadyIncluded) {
        events.push({
          ...
          partner: isSender ? allTargets.join("、") : group.sendname,
...
sorted.forEach(ev => {
  ...
  ev.displayText = `【${ev.day} ${ev.time}】\n` +
                   `${typeIcon} ${ev.subtype} · ${statusLabel}${replyTag}\n` +
                   `📍 地点：${ev.place || "未知"}\n` +
                   `👥 伙伴：${ev.partner}`;
});

// 以合并转发形式发送所有日程
sendTimelineCombined(ctx, msg, sorted, platform, myRoleName, a_private_group);
return seal.ext.newCmdExecuteResult(true);
};

function sendTimelineCombined(ctx, msg, events, platform, myRoleName, a_private_group) {
  if (!msg.groupId) {
    seal.replyToSender(ctx, msg, "私聊暂不支持合并转发，请在群内使用。");
    return;
  }
  const targetGid = msg.groupId.replace(/[^\d]/g, "");
  const botUid = ctx.endPoint.userId; // 机器人QQ（需确保正确）

  // 根据角色名查找真实QQ
  const getUidByRole = (roleName) => {
    const addr = a_private_group[platform]?.[roleName];
  ...
  events.forEach(ev => {
    ...
    // --- 确定发送者 ---
    let senderName = "日程助手";
    let senderUid = botUid;

    if (ev.partner) {
      // 取伙伴列表中的第一个角色名（以 "、" 或 "," 分割）
      const partners = ev.partner.split(/[、,]/).map(s => s.trim()).filter(s => s);
      if (partners.length > 0) {
        const firstPartner = partners[0];
        const uid = getUidByRole(firstPartner);
        if (uid) {
          // 有真实QQ：完美，使用伙伴角色名和真实QQ
          senderName = firstPartner;
          senderUid = uid;
        } else {
          // 无对应QQ：仍使用伙伴角色名，但用机器人QQ（头像显示为机器人，名字显示角色名）
          senderName = firstPartner;
          senderUid = botUid;
        }
      }
    }
    // 若 partner 为空或无法解析，则保持默认的“日程助手”

    // 使用预计算的 displayText（已在 solve 中生成）
    nodes.push({
      type: "node",
      data: {
        name: senderName,
        uin: senderUid,
        content: ev.displayText || "日程信息缺失"
      }
    });
  });
  ...
  nodes.unshift({
    ...
    data: {
      name: "时间线档案",
      uin: botUid,
      content: `📅 以下是您的日程明细（共 ${events.length} 条）：`
    }
  });
