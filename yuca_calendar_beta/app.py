...
_default_rp_db = os.path.join(os.path.dirname(__file__), '..', 'rp_archive', 'rp_data.db')
RP_ARCHIVE_DB  = os.environ.get("RP_ARCHIVE_DB", os.path.normpath(_default_rp_db))
...
def _migrate(conn):
    ...
    if 'arc_length'   not in cols_u: conn.execute("ALTER TABLE users ADD COLUMN arc_length TEXT NOT NULL DEFAULT ''")
    if 'online_hours' not in cols_u: conn.execute("ALTER TABLE users ADD COLUMN online_hours TEXT NOT NULL DEFAULT ''")
    if 'is_gm'        not in cols_u: conn.execute("ALTER TABLE users ADD COLUMN is_gm INTEGER NOT NULL DEFAULT 0")
    if 'qq'           not in cols_u: conn.execute("ALTER TABLE users ADD COLUMN qq TEXT NOT NULL DEFAULT ''")
    cols_d = _col_names(conn, 'dramas') if conn.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='dramas'").fetchone() else set()
    if 'rp_sync_key' not in cols_d: conn.execute("ALTER TABLE dramas ADD COLUMN rp_sync_key TEXT DEFAULT NULL")
    ...
    conn.execute('''CREATE TABLE IF NOT EXISTS gm_applications (
        id          INTEGER PRIMARY KEY AUTOINCREMENT,
        user_id     INTEGER NOT NULL REFERENCES users(id),
        reason      TEXT NOT NULL DEFAULT '',
    ...
    conn.execute('''CREATE TABLE IF NOT EXISTS applications (
        ...
        arc_length         TEXT NOT NULL DEFAULT '',
        online_hours       TEXT NOT NULL DEFAULT '',
        drama_refs         TEXT NOT NULL DEFAULT '',
        note               TEXT NOT NULL DEFAULT '',
        status             TEXT NOT NULL DEFAULT 'pending',
        player_schedule_id INTEGER,
        created_at         INTEGER NOT NULL
    )''')
    conn.commit()
...
def require_gm(f):
    ...
    def wrapped(*a, **kw):
        if not session.get('user_id'):
            return redirect(url_for('login', mode='gm'))
        user = get_db().execute('SELECT is_gm FROM users WHERE id=?', (session['user_id'],)).fetchone()
        if not user or not user['is_gm']:
            flash('需要本主权限，请先申请')
            return redirect(url_for('apply_gm'))
        return f(*a, **kw)
    return wrapped
...
def login():
    if session.get('user_id'):
        return redirect(url_for('gm_index') if session.get('login_mode') == 'gm' else url_for('calendar'))
    if request.method == 'POST':
        username = request.form.get('username', '').strip()
        password = request.form.get('password', '')
        mode     = request.form.get('login_mode', 'player')
        user = get_db().execute(
            'SELECT * FROM users WHERE username=? AND is_active=1', (username,)
        ).fetchone()
        if user and check_password_hash(user['password_hash'], password):
            if mode == 'gm' and not user['is_gm']:
                return render_template('login.html', active_mode='gm',
                    error='您还没有本主权限，请先在玩家端设置页申请')
            session.update({'user_id': user['id'], 'username': user['username'],
                            'display_name': user['display_name'] or user['username'],
                            'is_gm': user['is_gm'], 'login_mode': mode})
            return redirect(url_for('gm_index') if mode == 'gm' else url_for('calendar'))
        return render_template('login.html', active_mode=request.form.get('login_mode', 'player'),
                               error='用户名或密码错误，或账号已停用')
    active_mode = request.args.get('mode', 'player')
    return render_template('login.html', active_mode=active_mode)
...
def settings():
    ...
    if request.method == 'POST':
        ...
        elif action == 'arc_profile':
            arc_length   = request.form.get('arc_length', '').strip()
            online_hours = request.form.get('online_hours', '').strip()
            qq           = request.form.get('qq', '').strip()
            db.execute('UPDATE users SET arc_length=?, online_hours=?, qq=? WHERE id=?',
                       (arc_length, online_hours, qq, uid))
            db.commit()
            user = dict(db.execute('SELECT * FROM users WHERE id=?', (uid,)).fetchone())
            success = '投递信息已更新'
...
@app.route('/sync-rp', methods=['POST'])
@require_login
def sync_rp_history():
    uid  = session['user_id']
    db   = get_db()
    ...
    if not os.path.exists(RP_ARCHIVE_DB):
        flash('⚠ 未找到录档数据库，请确认路径配置')
        return redirect(url_for('settings'))

    try:
        rp_conn = sqlite3.connect(RP_ARCHIVE_DB)
        rp_conn.row_factory = sqlite3.Row
        rows = rp_conn.execute('''
            SELECT p.show_id, p.qq, p.role_name,
                   p.sessions_count, p.total_words,
                   s.name   AS season_name,
                   t.display_name AS tenant_name
            FROM players p
            JOIN shows   s ON s.id        = p.show_id
            JOIN tenants t ON t.id        = p.tenant_id
            WHERE p.qq = ? AND p.sessions_count > 0
            ORDER BY p.last_updated DESC
        ''', (qq,)).fetchall()
        rp_conn.close()
    except Exception as e:
        _logger.error("sync_rp_history: %s", e)
        flash('⚠ 读取录档数据库失败')
        return redirect(url_for('settings'))
    ...
    ts = int(datetime.now().timestamp())
    for r in rows:
        sync_key = f"rp:{r['show_id']}:{qq}"
        title    = f"{r['tenant_name']} · {r['season_name']}" if r['tenant_name'] else r['season_name']
        content  = f"角色：{r['role_name']}\n场次：{r['sessions_count']}\n字数：{r['total_words'] or 0}"

        existing = db.execute('SELECT id FROM dramas WHERE user_id=? AND rp_sync_key=?',
                              (uid, sync_key)).fetchone()
        if existing:
            db.execute('''UPDATE dramas SET title=?, content=?, writing_time=?, updated_at=?
                          WHERE id=?''',
                       (title, content, r['total_words'] or 0, ts, existing['id']))
            updated += 1
        else:
            db.execute('''INSERT INTO dramas (user_id, title, tags, content, writing_time, rp_sync_key, created_at, updated_at)
                          VALUES (?,?,?,?,?,?,?,?)''',
                       (uid, title, json.dumps(['从录档同步']), content,
                        r['total_words'] or 0, sync_key, ts, ts))
            added += 1

    db.commit()
    flash(f'✅ 同步完成：新增 {added} 条，更新 {updated} 条（共 {len(rows)} 条录档记录）')
    return redirect(url_for('settings'))


# ── 申请成为本主 ────────────────────────────────────────────────────────────────

@app.route('/apply-gm', methods=['GET', 'POST'])
@require_login
def apply_gm():
    uid = session['user_id']
    db  = get_db()
    ...
    pending = db.execute(
        "SELECT * FROM gm_applications WHERE user_id=? AND status='pending'", (uid,)
    ).fetchone()
    if request.method == 'POST' and not pending:
        reason = request.form.get('reason', '').strip()
        db.execute('INSERT INTO gm_applications (user_id, reason, status, created_at) VALUES (?,?,?,?)',
                   (uid, reason, 'pending', int(datetime.now().timestamp())))
        db.commit()
        flash('✅ 申请已提交，等待超管审批')
        return redirect(url_for('settings'))
    return render_template('apply_gm.html', pending=pending, user=user)
...
def my_applications():
    ...
    db  = get_db()
    apps = db.execute('''
        SELECT a.*, r.title AS rec_title, r.start_year AS rec_year, r.time_range AS rec_range,
...
def plaza():
    db = get_db()
    recs = db.execute('''
        SELECT r.*, u.display_name AS gm_name,
               COUNT(a.id) AS applicant_count
        FROM recruitments r
        JOIN users u ON u.id = r.gm_id
        LEFT JOIN applications a ON a.recruitment_id = r.id AND a.status != 'rejected'
        WHERE r.status = 'open'
        GROUP BY r.id
        ORDER BY r.created_at DESC
    ''').fetchall()
    return render_template('plaza.html', recs=recs)


@app.route('/plaza/<int:rid>', methods=['GET', 'POST'])
def plaza_detail(rid):
    db  = get_db()
    rec = db.execute('''
        SELECT r.*, u.display_name AS gm_name, u.username AS gm_username
        FROM recruitments r JOIN users u ON u.id=r.gm_id
        WHERE r.id=?
    ''', (rid,)).fetchone()
    if not rec:
        abort(404)
    ...
    user_info = db.execute('SELECT * FROM users WHERE id=?', (uid,)).fetchone() if uid else None
    dramas    = db.execute('SELECT id, title FROM dramas WHERE user_id=? ORDER BY created_at DESC',
                           (uid,)).fetchall() if uid else []

    already_applied = None
    if uid:
        already_applied = db.execute(
            "SELECT id FROM applications WHERE recruitment_id=? AND user_id=? AND status!='rejected'",
            (rid, uid)
        ).fetchone()
    ...
    if request.method == 'POST':
        if rec['status'] != 'open':
            flash('该招募已关闭')
            return redirect(url_for('plaza_detail', rid=rid))

        name         = request.form.get('applicant_name', '').strip()
        arc_length   = request.form.get('arc_length', '').strip()
        online_hours = request.form.get('online_hours', '').strip()
        drama_ids    = request.form.getlist('drama_ids')
        drama_text   = request.form.get('drama_text', '').strip()
        note         = request.form.get('note', '').strip()

        if not name:         errors.append('请填写称呼')
        if not arc_length:   errors.append('请填写弧长')
        if not online_hours: errors.append('请填写在线时间')
        if already_applied:  errors.append('您已投递过该招募')

        drama_refs = json.dumps(drama_ids) if drama_ids else drama_text
            ...
            if uid:
                cur = db.execute(
                    '''INSERT INTO schedules
                       (user_id, show_name, start_year, time_range, status, character, orientation,
                        theme, role_name, role_type, gender, outcome, color, notes,
                        schedule_tags, casting_status, created_at, updated_at)
                       VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)''',
                    (uid, rec['title'], rec['start_year'], rec['time_range'],
                     '进行中', '未知', '未知', '未知', '未知', '未知', '未知', '未知',
                     '', f'来自招募广场：{rec["title"]}',
                     json.dumps(['下组']), '一表中', ts, ts)
                )
                sched_id = cur.lastrowid

            db.execute(
                '''INSERT INTO applications
                   (recruitment_id, user_id, applicant_name, arc_length, online_hours,
                    drama_refs, note, status, player_schedule_id, created_at)
                   VALUES (?,?,?,?,?,?,?,?,?,?)''',
                (rid, uid, name, arc_length, online_hours, drama_refs, note, 'pending', sched_id, ts)
            )
            db.commit()
            flash('✅ 投递成功！' + ('已加入档期「下组/一表中」' if sched_id else ''))
            return redirect(url_for('plaza_detail', rid=rid))

    return render_template('plaza_detail.html', rec=rec, user_info=user_info, dramas=dramas,
                           already_applied=already_applied, errors=errors)
...
def gm_index():
    ...
    db  = get_db()
    recs = db.execute('''
        SELECT r.*, COUNT(a.id) AS total_apps,
               SUM(CASE WHEN a.status='pending' THEN 1 ELSE 0 END) AS pending_apps
        FROM recruitments r
        LEFT JOIN applications a ON a.recruitment_id = r.id
        WHERE r.gm_id=?
        GROUP BY r.id ORDER BY r.created_at DESC
    ''', (uid,)).fetchall()
    return render_template('gm_index.html', recs=recs)


@app.route('/gm/recruit/new', methods=['GET', 'POST'])
@require_gm
def gm_recruit_new():
    uid    = session['user_id']
    db     = get_db()
    ...
    if request.method == 'POST':
        ...
        if not errors:
            ts = int(datetime.now().timestamp())
            cur = db.execute(
                'INSERT INTO recruitments (gm_id, title, description, start_year, time_range, status, created_at) VALUES (?,?,?,?,?,?,?)',
                (uid, title, description, start_year, time_range, 'open', ts)
            )
            rid = cur.lastrowid
            # 本主日历自动加一条带本/已录取档期
            sc = db.execute(
                '''INSERT INTO schedules
                   (user_id, show_name, start_year, time_range, status, character, orientation,
                    ...
                    schedule_tags, casting_status, created_at, updated_at)
                   VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)''',
                (uid, title, start_year, time_range,
                 '进行中', '未知', '未知', '未知', '本主', '未知', '未知', '未知',
                 '', f'招募广场招募（#{rid}）',
                 json.dumps(['带本']), '已录取', ts, ts)
            )
            db.execute('UPDATE recruitments SET gm_schedule_id=? WHERE id=?', (sc.lastrowid, rid))
            db.commit()
            flash(f'✅ 招募「{title}」已发布，档期已加入日历')
            return redirect(url_for('gm_recruit_detail', rid=rid))

    now = datetime.now()
    return render_template('gm_recruit_form.html', errors=errors, form=request.form,
                           default_year=now.year)


@app.route('/gm/recruit/<int:rid>')
@require_gm
def gm_recruit_detail(rid):
    uid = session['user_id']
    db  = get_db()
    rec = db.execute('SELECT * FROM recruitments WHERE id=? AND gm_id=?', (rid, uid)).fetchone()
    if not rec:
        abort(404)
    apps = db.execute('''
        SELECT a.*, u.display_name AS player_name, u.username AS player_username
        FROM applications a
        LEFT JOIN users u ON u.id = a.user_id
        WHERE a.recruitment_id=?
        ORDER BY a.created_at ASC
    ''', (rid,)).fetchall()
    return render_template('gm_recruit_detail.html', rec=rec, apps=apps)


@app.route('/gm/recruit/<int:rid>/accept/<int:aid>', methods=['POST'])
@require_gm
def gm_recruit_accept(rid, aid):
    uid = session['user_id']
    db  = get_db()
    rec = db.execute('SELECT * FROM recruitments WHERE id=? AND gm_id=?', (rid, uid)).fetchone()
    if not rec:
        abort(404)
    app_ = db.execute('SELECT * FROM applications WHERE id=? AND recruitment_id=?', (aid, rid)).fetchone()
    if not app_:
        abort(404)
    db.execute("UPDATE applications SET status='accepted' WHERE id=?", (aid,))
    if app_['player_schedule_id']:
        db.execute("UPDATE schedules SET casting_status='已录取', updated_at=? WHERE id=?",
                   (int(datetime.now().timestamp()), app_['player_schedule_id']))
    db.commit()
    flash(f'✅ 已录取「{app_["applicant_name"]}」')
    return redirect(url_for('gm_recruit_detail', rid=rid))


@app.route('/gm/recruit/<int:rid>/reject/<int:aid>', methods=['POST'])
@require_gm
def gm_recruit_reject(rid, aid):
    uid = session['user_id']
    db  = get_db()
    rec = db.execute('SELECT * FROM recruitments WHERE id=? AND gm_id=?', (rid, uid)).fetchone()
    if not rec:
    ...
    if app_['player_schedule_id']:
        db.execute('DELETE FROM schedules WHERE id=?', (app_['player_schedule_id'],))
    db.commit()
    flash(f'已拒绝「{app_["applicant_name"]}」的投递，对应档期已删除')
    return redirect(url_for('gm_recruit_detail', rid=rid))


@app.route('/gm/recruit/<int:rid>/close', methods=['POST'])
@require_gm
def gm_recruit_close(rid):
    uid = session['user_id']
    db  = get_db()
    rec = db.execute('SELECT * FROM recruitments WHERE id=? AND gm_id=?', (rid, uid)).fetchone()
    if not rec:
        abort(404)
    db.execute("UPDATE recruitments SET status='closed' WHERE id=?", (rid,))
    db.commit()
    flash('招募已关闭')
    return redirect(url_for('gm_recruit_detail', rid=rid))


@app.route('/gm/recruit/<int:rid>/reopen', methods=['POST'])
@require_gm
def gm_recruit_reopen(rid):
    uid = session['user_id']
    db  = get_db()
    rec = db.execute('SELECT * FROM recruitments WHERE id=? AND gm_id=?', (rid, uid)).fetchone()
    if not rec:
        abort(404)
    db.execute("UPDATE recruitments SET status='open' WHERE id=?", (rid,))
    db.commit()
    flash('招募已重新开放')
    return redirect(url_for('gm_recruit_detail', rid=rid))
...
@app.route('/superadmin/gm-applications')
@require_superadmin
def superadmin_gm_applications():
    db   = get_db()
    apps = db.execute('''
        SELECT ga.*, u.username, u.display_name
        FROM gm_applications ga JOIN users u ON u.id=ga.user_id
        ORDER BY CASE ga.status WHEN 'pending' THEN 0 ELSE 1 END, ga.created_at DESC
    ''').fetchall()
    return render_template('superadmin_gm_apps.html', apps=apps)


@app.route('/superadmin/gm-applications/<int:aid>/approve', methods=['POST'])
@require_superadmin
def superadmin_approve_gm(aid):
    db  = get_db()
    app_ = db.execute('SELECT * FROM gm_applications WHERE id=?', (aid,)).fetchone()
    if not app_:
        abort(404)
    db.execute("UPDATE gm_applications SET status='approved', reviewed_at=? WHERE id=?",
               (int(datetime.now().timestamp()), aid))
    db.execute('UPDATE users SET is_gm=1 WHERE id=?', (app_['user_id'],))
    db.commit()
    flash('✅ 已批准本主申请')
    return redirect(url_for('superadmin_gm_applications'))


@app.route('/superadmin/gm-applications/<int:aid>/reject', methods=['POST'])
@require_superadmin
def superadmin_reject_gm(aid):
    db   = get_db()
    app_ = db.execute('SELECT * FROM gm_applications WHERE id=?', (aid,)).fetchone()
    if not app_:
        abort(404)
    db.execute("UPDATE gm_applications SET status='rejected', reviewed_at=? WHERE id=?",
               (int(datetime.now().timestamp()), aid))
    db.commit()
    flash('已拒绝本主申请')
    return redirect(url_for('superadmin_gm_applications'))
